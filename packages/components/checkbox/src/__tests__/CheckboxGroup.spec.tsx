...
describe('<CheckboxGroup />', () => {
  ...
  /*   it('renders the default error icon when errorIcon is not provided', () => {
    render(
      <CheckboxGroup
        name="checkboxGroup"
        options={options}
    ...
    const defaultIcon = screen.getByRole('img', { name: 'chevron-up' });
    expect(defaultIcon).toBeInTheDocument();
  });

  it('renders the provided errorIcon when errorIcon is passed', () => {
    render(
      <CheckboxGroup
        name="checkboxGroup"
        options={options}
        error="This is an error"
        errorIcon={<Icon name="close" />}
      />,
    );

    const customIcon = screen.getByRole('img', { name: 'close' });
