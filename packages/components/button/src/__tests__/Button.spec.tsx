...
describe('<Button />', () => {
  ...
  it('is primary button when typeBtn prop is primary', () => {
    render(<Button label="Button 1" typeBtn='primary' />);

    console.log(getComputedStyle(screen.getByText('Button 1')).backgroundColor);

    expect(screen.getByText('Button 1')).toHaveStyle('background-color: #298DCC');
  });

  it('is secondary button when typeBtn prop is secondary', () => {
    render(<Button label="Button 1" typeBtn='secondary' />);

    expect(screen.getByText('Button 1')).toHaveStyle('background-color: #FFFFFF');
  });

  it('is error button when typeBtn prop is error', () => {
    render(<Button label="Button 1" typeBtn='error' />);

    expect(screen.getByText('Button 1')).toHaveStyle('background-color: #B24747');
  });
